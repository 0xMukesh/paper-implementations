...
class ResNeXtBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, base_out_channels: int, cardinality: int = 32, width: int = 4, stride: int = 1):
        super().__init__()
        ...
        if stride != 1 or in_channels != base_out_channels * self.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=base_out_channels * self.expansion, kernel_size=1, stride=stride),
...
class ResNeXt50(nn.Module):
    def __init__(self, block, classes: int, in_channels: int = 3):
        ...
        self.blocks = self._make_blocks([3, 4, 6, 3], base_in_channels=64, base_out_channels=64, base_group_width=4, cardinality=32)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(in_features=512 * block.expansion, out_features=classes)

    def _make_blocks(self, arch: list[int], base_in_channels: int, base_out_channels: int, base_group_width: int, cardinality: int):
        layers = []
        ...
        for i, x in enumerate(arch):
            ...
            blocks.append(self.block(in_channels=in_channels, base_out_channels=out_channels, cardinality=cardinality, width=width, stride=stride))
